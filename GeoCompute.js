...
var converter = new SVY21();
var northing = 31490.4942;
var easting = 30314.7936;
var result = converter.computeLatLon(northing, easting);

const obj = csv(); 
function CarparkDetails(Fone, Ftwo, Fthree,Ffour, Ffive, Fsix) {
    this.address = Fone;
    this.carpark_Type = Ftwo;
    this.payment_mode = Fthree;
    this.lat = Ffour;
    this.long = Ffive;
    this.shortterm_parking = Fsix;
    this.distance = null;
}; 

var MyData = []; 
obj.from.path('./assets/hdbCarpark.csv').to.array(function (data) {
    for (var index = 0; index < data.length; index++) {
        result = converter.computeLatLon(data[index][3], data[index][2]);
        MyData.push(new CarparkDetails(data[index][1], data[index][4], data[index][5], result.lat, result.lon, data[index][6]));
    }
});


// console.log("Distance in KM:", converter.calcDistance(result.lat, result.lon, 1.3885775999999999, 103.90624419999999,"K"));
// console.log(result.lat, result.lon);

//exporting of the function object
module.exports = {converter, MyData};